    ...
    <div class="dark:bg-slate-900 bg-slate-50 duration-100">
        <main class="flex flex-col justify-center text-center sm:text-left w-full sm:max-w-4xl sm:mx-auto">
            <nav class="grid grid-cols-2 gap-x-4 mx-4 sm:mx-0 sm:gap-x-0 sm:flex sm:flex-row items-center text-slate-50 text-md my-4 sm:my-8 sm:my-12 font-semibold">
                ...
                <a href="/#projects" class="flex flex-row items-center my-2 sm:my-0 sm:mx-2 px-4 py-1 dark:bg-slate-600 bg-slate-300 rounded-md dark:hover:bg-slate-700 hover:bg-slate-400 dark:text-slate-50 text-slate-800">
                    <Projects />
                    <span class="pl-2">Projects</span>
                </a>
                <a href="/#socials" class="flex flex-row items-center my-2 sm:my-0 sm:mx-2 px-4 py-1 dark:bg-slate-600 bg-slate-300 rounded-md dark:hover:bg-slate-700 hover:bg-slate-400 dark:text-slate-50 text-slate-800">
                    <Socials />
                    <span class="pl-2">Socials</span>
                </a>
                <button class="flex flex-row items-center my-2 sm:my-0 sm:mx-2 px-4 py-1 dark:bg-slate-600 bg-slate-300 rounded-md dark:hover:bg-slate-700 hover:bg-slate-400 dark:text-slate-50 text-slate-800" on:click={toggleTheme}>
                    {#if dark}
                        <Moon />
                    {:else}
                        <Sun />
